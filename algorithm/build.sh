...
for s in "${services[@]}"; do
    echo "服务 $s 构建开始."
    python -m nuitka --nofollow-imports --enable-plugin=multiprocessing --include-package=AskOnce --output-dir=build/  --static-libpython=no -o "build/$s.bin" --remove-output "services/$s/$s.py"

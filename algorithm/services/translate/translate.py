...
from AskOnce.algorithm.lib.llm_api.translate import Translate
import time
...
class DataInput:
    def __init__(self,json_data,task_id) -> None:
        self.input_text = json_data['input_text']
        self.target_language = json_data['target_language']
...
def process(task_input,task_type,model,args,tm):
    if task_type ==args.tasktype[0]:
        ...
        result_all = {} 
        result_all['result'] = model.translate(task_input.input_text,task_input.target_language)
        end_time = time.time()
        print('翻译','用时'+str(end_time-start_time))
        return result_all
...
if __name__ == '__main__':
    ...
    model = Translate(api_key=args.api_key,platform_api_url=args.platform_api_url,model_name= args.model_name)    
